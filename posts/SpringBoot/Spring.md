...
- 
  ...
  **4. AOP (Aspect Oriented Programming)**
    ...
    - **Spring AOP:** 관점 지향 프로그래밍으로 애플리케이션의 핵심 비즈니스 로직과 로깅, 트랜잭션 같은 **공통 관심사를 분리**하는 기술입니다.
      
       
       기존에는 여러 메서드에 중복되는 공통 코드를 일일이 작성해야 했지만 AOP를 사용하면 이를 **Aspect**라는 독립적인 모듈로 관리할 수 있어 코드의 재사용성과 유지보수성이 크게 향상됩니다. 내부적으로는 **프록시 패턴**을 사용하여 동작합니다. 런타임 시점에 스프링이 빈(Bean) 객체를 감싸는 프록시 객체를 생성하고 설정된 **Pointcut**에 따라 비즈니스 로직 전후에 **Advice**를 주입하는 방식입니다. 대표적인 예로는 `@Transactional` 어노테이션을 통해 선언적으로 트랜잭션을 관리하는 기능을 들 수 있습니다.
        
       - Spring AOP는 **핵심 비즈니스 로직과 공통 관심사(Cross-cutting Concerns)를 분리**하여 코드의 재사용성과 유지보수성을 높이는 프로그래밍 패러다임
       - **코드 중복 제거:** 로깅, 트랜잭션 관리, 보안 인증 같은 기능은 여러 서비스에서 공통적으로 쓰임 이를 각 로직에 직접 넣으면 코드가 복잡해지는데 AOP를 통해 이를 한곳에서 관리
       - **비즈니스 로직 집중:** 개발자가 핵심 로직에만 집중할 수 있게 하여 코드 가독성을 높임
       - **Aspect:** 공통 관심사를 모듈화한 것 (예: 로깅 기능을 담은 클래스)
       - **Target:** Aspect를 적용할 대상 (비즈니스 로직을 담은 클래스나 메서드)
       - **Advice:** 실질적으로 어떤 동작을 할 것인지에 대한 정의 (언제 실행할지: Before, After, Around 등)
       - **Pointcut:** Advice를 적용할 지점(메서드 범위 등)을 선정하는 규칙
       - **Join Point:** Advice가 적용될 수 있는 실행 지점 (Spring에서는 주로 '메서드 실행 시점')
       - Spring AOP는 **프록시 패턴**을 기반으로 동작
    
  **4-1. 프록시 패턴이란?**
    
    - 프록시 패턴은 **특정 객체에 대한 접근을 제어하거나 기능을 추가하기 위해 그 객체의 인터페이스를 대신 구현하는 대리 객체를 두는 패턴**
        
       클라이언트는 실제 객체(Real Subject)를 직접 호출하는 것이 아니라 실제 객체와 똑같은 인터페이스를 가진 **프록시(Proxy)**를 호출하게 됨
        
       | **종류** | **설명** | **주요 사용 사례** |
        | --- | --- | --- |
        | **가상 프록시 (Virtual)** | 실제 객체의 생성 비용이 클 때, **필요한 시점까지 생성을 미룸** | JPA 지연 로딩, 대용량 이미지 로딩 |
        | **보호 프록시 (Protection)** | 객체에 대한 **접근 권한을 제어** | Spring Security, 등급별 메뉴 접근 |
        | **원격 프록시 (Remote)** | 다른 서버에 있는 객체를 **로컬에 있는 것처럼** 호출 | gRPC, Feign Client, RMI |
        | **스마트 프록시 (Smart Reference)** | 객체 접근 시 추가적인 작업을 수행 | 로깅, 캐싱, 성능 측정, 참조 횟수 계산 |
    
    프록시 패턴을 쓰는 이유는 **"핵심 로직은 건드리지 않으면서 주변에 필요한 부가 기능(보안, 로그, 트랜잭션 등)을 유연하게 붙였다 뗐다 하기 위해서"**입니다.
...
### 6-1. `@RequiredArgsConstructor` 를 쓰는 이유
   ...
   - **그래서 `@RequiredArgsConstructor`를 쓴다?** → 생성자 코드 짜기 귀찮으니까, 롬복한테 "final 붙은 애들 모아서 생성자 만들어줘!"라고 시키는 것입니다.
    ...
    스프링에서는 **생성자 주입**이 테스트 용이성과 불변성 때문에 권장되는데요. 생성자 코드를 매번 직접 작성하면 코드가 길어지고 번거롭기 때문에 **Lombok의 `@RequiredArgsConstructor`를 사용하여 반복적인 코드를 줄이고 깔끔하게 의존성을 주입**받고 있습니다.
...
# 18. **DispatcherServlet이란 무엇인가요?**


    
- **DispatcherServlet**은 Spring MVC의 **가장 앞단에서 모든 요청을 받는 단일 진입점(Front Controller)** 역할을 수행합니다.
    
- **DispatcherServlet**은 Spring MVC의 핵심인 프론트 컨트롤러(Front Controller)로서 애플리케이션으로 들어오는 모든 HTTP 요청을 가장 먼저 받아 적절한 컨트롤러로 배분하고 처리된 결과를 바탕으로 최종 응답을 생성하는 **중앙 처리 장치** 역할을 수행합니다

# 19. Filter와 Interceptor의 차이는?

- 
    
    Filter는 DispatcherServlet 실행 전(Web Context)에 동작하며 인코딩 등에 사용됩니다. Interceptor는 DispatcherServlet과 Controller 사이(Spring Context)에 동작하며 인증/인가 등에 사용됩니다.
    
    - **Filter가 필요한 이유:** 스프링(Spring)이라는 프레임워크가 시작되기도 전에 처리해야 할 일들이 있음. 예를 들어 → 요청 데이터가 깨지지 않게 인코딩을 맞추는 일은 스프링이 데이터를 읽기 전에 무조건 먼저 해야 함
    - **Interceptor가 필요한 이유:** "회원 등급 확인" 같은 일은 DB 조회가 필요하거나 구체적인 비즈니스 로직이 섞여 있음. 이는 스프링이 제공하는 기능들을 활용해야 하므로 스프링 내부에서 처리하는 것이 훨씬 유리